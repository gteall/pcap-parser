...
struct PcapPktHeader
{
    UInt secTime;
    UInt microTime;
    UInt capLen;    /** 抓包获取到的长度,如果抓包时限制了单个包的大小会小于actualLen */
    UInt actualLen;
};
...
public:
    ...
    {
        if((mSrcIp == id.getSrcIp())
            && (mDestIp == id.getDestIp())
            && (mSrcPort == id.getSrcPort())
    ...
    void setSrcIp(UInt ip)
    {
        mSrcIp = ip;
    }
    
    void setDestIp(UInt ip)
    {
        mDestIp = ip;
    }
...
private:
    int parseEthPktHeader(const char *pBuf, UInt bufLen, UInt &headerLen, UInt linkType);
